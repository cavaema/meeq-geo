...
// 🔥 MOD 2: Verify-pin con gender e distinctive_sign
app.post('/api/verify-pin', (req, res) => {
  const { email, pin, gender, distinctiveSign } = req.body;
  // 🔥 FIX: tavolo è opzionale, verrà impostato dopo con select-table
  const tavolo = req.body.tavolo || 'temp';

  if (!email || !pin) {  // 🔥 FIX: Non richiedo più tavolo come obbligatorio
    ...
    (err, pending) => {
      ...
      db.get('SELECT * FROM users WHERE email = ?', [email], (err2, existingUser) => {
        ...
        if (existingUser) {
          ...
          db.run(
            'UPDATE users SET logged_in = 1, tavolo = ?, gender = ?, distinctive_sign = ?, last_login = CURRENT_TIMESTAMP, last_activity = CURRENT_TIMESTAMP WHERE email = ?',
            [tavolo, updateGender, updateSign, email],
            function(updateErr) {
              if (updateErr) {
              ...
              // console.log(`✅ Utente ri-loggato: ${existingUser.nome} ${existingUser.cognome}`);

              res.json({
                token,
                userId: existingUser.id, // Aggiungo userId per il frontend
                user: {
                  id: existingUser.id,
          ...
          db.run(
            `INSERT INTO users (nome, cognome, email, telefono, tavolo, gender, distinctive_sign, logged_in, privacy_accepted, newsletter_accepted, last_login, last_activity) 
             VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
            [pending.nome, pending.cognome, pending.email, pending.telefono, tavolo, userGender, userSign, pending.privacy_accepted, pending.newsletter_accepted],
            function(insertErr) {
              if (insertErr) {
              ...
              res.json({
                token,
                userId: newUserId, // Aggiungo userId per il frontend
                user: {
                  id: newUserId,
                  nome: pending.nome,
...
// 🔥 FIX: Endpoint per verificare il token
app.get('/api/verify-token', authenticateToken, (req, res) => {
  res.json({ valid: true, userId: req.user.userId });
});

// 🔥 FIX: Endpoint per aggiornare il tavolo dopo il login
...
app.get('/api/conversations', authenticateToken, (req, res) => {
  ...
  // Query semplice che funziona
  db.all(
    `SELECT 
      c.*,
      u1.nome as user1_name,
      u1.cognome as user1_surname,
      u1.gender as user1_gender,
      u1.distinctive_sign as user1_distinctive_sign,
      u2.nome as user2_name,
      u2.cognome as user2_surname,
      u2.gender as user2_gender,
      u2.distinctive_sign as user2_distinctive_sign
